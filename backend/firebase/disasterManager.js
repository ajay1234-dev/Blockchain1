...
class DisasterManager {
  ...
  static async createDisaster(disasterData, creatorId) {
    try {
      // Validate input
      if (!disasterData.name || !disasterData.description || !disasterData.targetFunding) {
      ...
      await disasterRef.set({
        ...
        organizer: creatorId,
        startDate: disasterData.startDate || admin.firestore.FieldValue.serverTimestamp(),
  ...
  static async getActiveDisasters(limit = 10) {
    try {
      const snapshot = await db.collection(COLLECTIONS.DISASTERS)
        .where('status', '==', 'active')
    ...
    try {
      ...
      for (const [stat, value] of Object.entries(statUpdates)) {
        if (typeof value === 'number') {
          updateData[`metadata.stats.${stat}`] = admin.firestore.FieldValue.increment(value);
        } else {
