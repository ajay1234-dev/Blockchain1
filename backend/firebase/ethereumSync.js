...
class EthereumSync {
  ...
  async syncManagerEvents(fromBlock, toBlock) {
    try {
      ...
      const events = await this.managerContract.queryFilter(eventFilter, fromBlock, toBlock);
      ...
      for (const event of events) {
        ...
        await db.collection(COLLECTIONS.DISASTERS).doc(eventData.id).set({
          ...eventData,
          metadata: {
            stats: {
      ...
      const fundsDistributedFilter = this.managerContract.filters.FundsDistributed();
      ...
      for (const event of fundsDistributedEvents) {
        ...
        console.log(`Synced FundsDistributed: ${args.beneficiary} - ${args.amount}`);
      }
      ...
      for (const event of fundsSpentEvents) {
        ...
        console.log(`Synced FundsSpent: ${args.beneficiary} -> ${args.vendor} - ${args.amount}`);
      }

      // Sync BeneficiaryWhitelisted events
      const beneficiaryWhitelistFilter = this.managerContract.filters.BeneficiaryWhitelisted();
      ...
      const vendorWhitelistFilter = this.managerContract.filters.VendorWhitelisted();
      ...
      for (const event of vendorWhitelistEvents) {
        ...
        if (!existingVendor) {
          ...
          await db.collection(COLLECTIONS.VENDORS).doc(existingVendor.id).update({
            whitelisted: args.status,
            updatedAt: admin.firestore.FieldValue.serverTimestamp()
          });
        }
        ...
        console.log(`Synced VendorWhitelisted: ${args.vendor} - ${args.status}`);
      }
  ...
  async syncStablecoinEvents(fromBlock, toBlock) {
    try {
      ...
      const categorySpentFilter = this.stablecoinContract.filters.CategorySpent();
      ...
      for (const event of categorySpentEvents) {
        ...
        console.log(`Synced CategorySpent: ${args.beneficiary} -> ${args.vendor} - ${category} - ${args.amount}`);
      }
  ...
  async startEventListener() {
    try {
      ...
      this.managerContract.on('EmergencyEventCreated', async (eventId, name, targetFunding, event) => {
        try {
          const eventData = {
            id: eventId.toString(),
            name: name,
          ...
          console.error('Error processing EmergencyEventCreated event:', error);
        }
      });

      // Listen for new FundsDistributed events
      this.managerContract.on('FundsDistributed', async (beneficiary, amount, eventId, event) => {
        try {
          const transactionData = {
            id: `tx_${event.transactionHash}_${event.logIndex}`,
            type: 'distribution',
          ...
          console.log(`Real-time sync: FundsDistributed: ${beneficiary} - ${amount}`);
        } catch (error) {
      ...
      this.managerContract.on('FundsSpent', async (beneficiary, vendor, amount, category, event) => {
        try {
          ...
          console.log(`Real-time sync: FundsSpent: ${beneficiary} -> ${vendor} - ${amount}`);
        } catch (error) {
